...
df2 <- dplyr::select(df, d28_death, calc_age ,ptsex , hivstatus ,Haemoglobin, screentemp ,t0map,
      ...
      tb, bc, malaria,any_tb_rx, any_fung_rx, any_mal_rx)
...
glm(d28_death ~ calc_age + ptsex + hivstatus + Haemoglobin + 
  ...
  brms::brm(d28_death ~ calc_age + ptsex + hivstatus + Haemoglobin + screentemp +  t0map + 
       t0spo2 + gcs + lactate_value + CO2 + Urea + Creatinine + 
      tb + malaria + bc +  any_tb_rx + any_fung_rx + any_mal_rx, data = df2.complete, family = "bernoulli", prior = m1priors) -> m.brms
