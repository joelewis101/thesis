...
rbind(
  ...
  pretty_tbl_df(
    select(e1, screentemp, Haemoglobin, WCC, Neutrophils_count,  Potassium, Urea)
    ,r = 1),
  pretty_tbl_df( 
    select(e1, Platelets, Chloride, CO2, Creatinine )
    )
  ) -> e1.sum.3
...
e1.sum.3 <- e1.sum.3[c(10,1:5,20,6:8, 9,11,12,13,16,14,18,17,15,19),]
...
kable(select(e1.sum.3,`levels`,value), "latex", booktabs =T, 
  ...
  add_indent(c(8:10)) %>%
  footnote(general = "GCS = Glasgow coma scale. Numeric values are median (IQR)) unless otherwise stated." , symbol = "CD4 count includes only HIV-infected participants.", threeparttable = T)
