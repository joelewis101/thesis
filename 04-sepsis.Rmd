...
Regression difficulties and solutions

colinearity/variable selection -> FAMD
nonidentifiability (zero cells) -> bayesian regression with weakly informative priors
...
Bivariate associations of mortality with host, disease severity and treatment variables are shown in \@ref(tab:outcome-morbidity)
...
names(df.famd.vars) <- c(  "hiv", "CD4", "age", "sex","Hb", "temp", "map", "hr", "rr", "spo2", "gcs","lactate",
    "WCC" , "platelets", "urea",  "creatinine", "bicarbonate", "ustand")
...
model.table$variable <- c("FAMD Dimension 1", "FAMD Dimension 2", "FAMD Dimension 3", "TB", "Malaria", "BSI", "Meningitis",
                          "TB Therapy", "Antifungals", "Antimalarials", "IV Fluid Recieved (per L)")


kable(model.table, "latex", booktabs =T,  
      row.names = F, longtable = F, ,escape = F,
      caption = "Adjusted odds ratios (aOR) for death by 28 days for complete case analysis (CCA) and multiply imputed data", col.names = c("Variable", "CCA", "Imputed") ) %>%
 kable_styling("striped", full_width = F, latex_options = c("repeat_header")) %>%
  add_header_above(c(" " = 1, "Adjusted Odds Ratio" = 2)) %>%
  pack_rows("FAMD composite variables", 1,3, bold = F) %>% 
   pack_rows("Diagnosis", 4, 7, bold = F)  %>%
    pack_rows("Therapies recieved", 8,11, bold = F) %>% 
  footnote(general = "FAMD = Factor Analysis of Mixed Data, BSI = Bloodstream infection, TB = Tuberculosis, CCA = Complete case analysis", threeparttable = T, fixed_small_size = T)



```

```{r bivariate-diag-sev, echo =F, warning=F, message=F, include = F}
# severity by diagnosis

df.temp2 <- dplyr::select(df, d28_death, hivstatus,hivonart,calc_age,ptsex,screentemp, t0sbp, t0dbp, t0map, t0hr, t0rr, t0spo2, gcs, ustand, lactate_value,
                         WCC, CD4_Absolute, Haemoglobin, Platelets, NA.,Potassium, Urea, Creatinine, CO2, Chloride, tb, malaria, bc, csf
) 
apply(df.temp2[c("tb", "malaria", "bc", "csf")], 1, sum) -> df.temp2$n_diagnoses
...
#df.temp2$diagnosis <- factor(apply(df.temp2[c("tb", "malaria", "bc", "no_diagnosis")], 1, function(x) which(x == 1)), 
 #      labels = colnames(df.temp2[c("tb", "malaria", "bc", "no_diagnosis")])) 
...
df.temp2$hivstatus <- as.numeric(df.temp2$hivstatus)
df.temp2$ptsex[df.temp2$ptsex == "Male"] <- 1
df.temp2$ptsex[df.temp2$ptsex == "Female"] <- 0
df.temp2$ptsex <- as.numeric(df.temp2$ptsex)

df.temp2$hivonart[df.temp2$hivonart == "Yes"] <- 1
df.temp2$hivonart[df.temp2$hivonart == "No"] <- 0
df.temp2$hivonart <- as.numeric(df.temp2$hivonart)


### new
df.temp2 %>% 
  select(-c(hivstatus, hivonart, ptsex, ustand, d28_death)) %>% pivot_longer(-c(malaria, tb,bc,no_diagnosis), names_to = "var_name", values_to = "var_val") %>% 
  pivot_longer(-c(var_name, var_val)) %>% filter(value == 1) %>%
  dplyr::group_by(var_name) %>% dplyr::summarise(pval = kruskal.test(var_val ~ name)$p.value) -> pvals

df.temp2 %>% select(-c(hivstatus, hivonart, ptsex, ustand, d28_death, csf, n_diagnoses)) %>% pivot_longer(-c(malaria, tb,bc,no_diagnosis), , names_to = "var_name", values_to = "var_val")  %>% 
  pivot_longer(-c(var_name, var_val)) %>% filter(value == 1) %>%
  group_by(name, var_name) %>%
  summarise(med = median(var_val, na.rm = T), 
            LQ = quantile(var_val, 0.25, na.rm = T),
            UQ = quantile(var_val, 0.75, na.rm = T)) -> p

###


paste0(
  specify_decimal(p$med, 1), " (",
  specify_decimal(p$LQ, 1), "-",
  specify_decimal(p$UQ, 1) ,")"
) -> p$med_str

p %>% select(name, var_name, med_str) %>% pivot_wider(names_from = name, values_from = med_str) -> p

p <- merge(p, pvals)

## new

df.temp2 %>% select( hivstatus, hivonart, ptsex, ustand, malaria, tb,bc, no_diagnosis) %>% pivot_longer(-c(malaria, tb,bc, no_diagnosis), names_to = "var_name", values_to = "var_val") %>% 
  pivot_longer(-c(var_name, var_val)) %>% filter(value == 1 & !is.na(var_val)) %>% 
  dplyr::group_by(name, var_name) %>% dplyr::summarise(`1` = sum(var_val == 1, na.rm = T), 
                                                        `0` = sum(var_val == 0, na.rm = T),
                                                        total = sum(!is.na(value)),
                                                        prop = `1`/total) %>% 
  group_by(var_name) %>% 
...
p.cat %>% select(name, var_name, str, pval) %>% pivot_wider(names_from = name, values_from = str) -> p.cat
...
df.temp2 %>% select(malaria, tb,bc, no_diagnosis) %>% 
  pivot_longer(everything()) %>%
  filter(value == 1) %>% group_by(name) %>% tally() %>%
  pivot_wider(names_from = name, values_from = n) -> n_d
...
n_d$boldcol <- FALSE
bind_rows(p, n_d) -> p
p[match(c("n","calc_age", "ptsex", "hivstatus","hivonart", "CD4_Absolute", "Haemoglobin","screentemp", "t0hr", "t0sbp", "t0dbp", "t0map",
        "t0rr", "t0spo2","gcs","ustand", "lactate_value" , "WCC", "Platelets",  "CO2", "Urea", "Creatinine"), p$var_name),] -> p

p <- p[c("var_name", "tb", "malaria","bc", "no_diagnosis", "pval", "boldcol")]
...
kable(select(p,-boldcol), "latex", booktabs =T, 
 ...
 kable_styling("striped", full_width = F, latex_options = c("repeat_header", "scale_down")) %>%
  row_spec(boldrows, bold = T) %>%
  pack_rows("Host Variables", 2,7, bold = F) %>% 
   pack_rows("Severity Variables", 8, 22, bold = F)  %>%
  footnote(general = "BP = Blood pressure, GCS = Glasgow coma scale. Numeric variables are presented as median (IQR) and categoric variables as proportions. P-values are from Kruskal-Wallace test for continuous variables and Fisher's exact test for categorical variables across four diganositic groups. Patients with meningitis are excluded from this table because of small numbers (n = 4)." , symbol = c("Participants with HIV status unknown not included in this row","HIV-infected participants only"), threeparttable = T, fixed_small_size = T) %>% add_indent(1) %>%
  landscape()

```


```{r outcome-by-diag, echo =F, warning=F, message=F,fig.scap="Mortality stratified by diagnosis and IV fluid therapy", out.extra='', fig.cap="A; Day 28 and 90 mortality stratified by diagnosis, B: Day 28 mortality stratified by IV fluid volume recieved in first 6hr of hospital admission.", fig.width= 6, fig.height =3, out.height= '70%' , fig.align = 'center'}
...
ggplot(d.mort, aes(fct_reorder(name,prop), prop, color = fct_reorder(type, prop))) + coord_flip() + geom_point(position = position_dodge(width = 0.3)) + 
  ...
  labs(y = "Mortality", x = "") + theme(legend.title = element_blank(), legend.position = "bottom") -> p.diag
...
ggarrange(p.diag, ggarrange(p.fluid,NULL, heights = c(0.90,0.1), labels = c("B", NA), nrow = 2), labels = c("A", NA), ncol = 2, widths = c(1.1,1))
...
```{r outcome-by-time-to-rx, echo =F, warning=F, message=F,fig.scap="Day 28 mortality stratified by time-to-antimicrobials", out.extra='', fig.cap="Day 28 mortality stratified by time-to-antimicrobials in hours (A-C). A and B show mortality stratified by time-to-antibacterial with different bin sizes to focus on the first 6hr (A) and afterwards (B). C shows mortality stratified by time to antituberculous therapy. In view of the small number of patients receiving antimalarial or antifungal therapy, this analysis was not repeated for those agents.", fig.width= 6, fig.height =6, out.height= '70%', fig.align= 'center'}
