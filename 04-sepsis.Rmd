...
```{r causal-structure, echo = F, fig.scap="Hypothesised causal structre of mortality in sepsis", out.extra='', fig.cap="Hypothesised causal structre of death in sepsis. Host factors would theoretcially include variables quantifying age, sex, immune status; infection would theoretcially include causative agents, site, and variables quantifying pathogen burden; severity would include includes measures of disease severity such as shock or respiratory failure; therapy would include which antimicrobials were administered and rapidity of administration of antimicrobials. Dotted edges from host and infection to outcome are because it is not clear \textit{a priori} whether the effect of infection and host factors are entirely mediated by disease severity: see text for details. ",  out.height = '50%', fig.align = 'center'}

knitr::include_graphics(paste0(wd, "figures/causal_dag_mortality.pdf"))

```

```{r bivariate-mort-assoc, echo =F, warning=F, message=F}
...
p$Variable[p$Variable == "CD4_Absolute"] <- "CD4 count ($\\mu$L\\textsuperscript{-1})*"
...
kable(select(p,-boldcol), "latex", booktabs =T, 
 ...
 kable_styling("striped", full_width = F, latex_options = c("repeat_header")) %>%
  ...
  footnote(general = "BP = Blood pressure, GCS = Glasgow coma scale. Numeric variables are presented as median (IQR) and categoric variables as proportions. P-values are from Kruskal-Wallace test for continuous variables and Fisher's exact test for categorical variables" , symbol = "CD4 count variable includes only HIV-infected participants", threeparttable = T, fixed_small_size = T)
