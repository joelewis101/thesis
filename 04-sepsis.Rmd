...
#bind_rows(
#v1 %>% group_by(hivstatus) %>% 
#  summarise(d28_n = sum(!is.na(d28_death)), 
 #   d28mort = prop_confint_str(d28_death), 
#            d90_n = sum(!is.na(d90_death)), 
#            d90mort = prop_confint_str(d90_death),
#            d180_n = sum(!is.na(d180_death)),
#            d180mort = prop_confint_str(d180_death)),
# v1 %>% ungroup %>% 
#  summarise(d28_n = sum(!is.na(d28_death)), 
#    d28mort = prop_confint_str(d28_death),
##               d90_n = sum(!is.na(d90_death)), 
...
#mort[4,1] <- "Total"
#mort[c(2,1,3,4),] -> mort
#data.frame(t(mort[,c(1,2,4,6)])) -> mort1

#data.frame(t(mort[,c(2,2,4,6)])) -> mort1

#names(mort) <- c("HIV+", "HIV-", "HIV Unknown", "Total")
#mort <- mort[-1,]

#mort$cats <- c("n", "Mortality", "n", "Mortality", "n", "Mortality")
#mort <- mort[,c(5,1,2,3,4)]

#kable(mort, "latex", booktabs = T, row.names = F,
#      caption = "Day 28, 90 and 180 mortality stratified by HIV status",
#       col.names = c("", names(mort)[2:5])) %>%
##        kable_styling( full_width = F) %>%
#        group_rows("Day 28", 1,2) %>%
#        group_rows("Day 90", 3,4) %>%
#        group_rows("Day 180", 5,6) %>%
...
psurv2 + theme(legend.position = "right", legend.title = element_blank()) + scale_fill_discrete(labels = c("HIV-", "HIV+")) + scale_color_discrete(labels = c("HIV-", "HIV+")) -> psurv2

ggarrange(psurv1, psurv2, labels = c("A", "B"), widths = c(0.8,1))
```


```{r outcome-morbidity, echo =F, warning=F, message=F,fig.scap="Health-related QoL following admission with sepsis.", out.extra='', fig.cap="Health-related quality of life following sepsis admission, compared to non-sepsis admissions, showing a return to usual QoL by 12 weeks in both sepsis and non-sepsis admissions. A: proportion of participants across each of the five domains of the EQ-5D questionnaire who report at least moderate impariment. B: Boxplots of visual analogue scale (VAS) score as a function of time. Dotted line marks median VAS score from community participants. To score on the VAS scale, participants are asked to mark their current global health status on a visual scale, which is converted into a 0-100 scale.", fig.width= 9, fig.height =5, }
...
eq %>% select(arm, visit_type, walk, self, usual, pain, anxiety) %>% 
  ...
  group_by(arm, visit_type, name) %>% summarise(x = sum(value >= 3), 
                                                n = n(), 
                                                prop = x / n,
...
eq.1 <- subset(eq.1, arm %in% c(1,2))
eq.1$arm[eq.1$arm == 1] <- "Sepsis"
eq.1$arm[eq.1$arm == 2] <- "Non-Sepsis"
eq.1$arm <- factor(eq.1$arm, levels = c("Sepsis", "Non-Sepsis"))
...
ggplot(eq.1, aes(visit_type,prop, color = arm, group = arm) ) + 
  ...
  theme(legend.title = element_blank()) + coord_cartesian(ylim = c(0,0.5)) -> p1 #+
 # scale_color_manual(values = c("black", "darkgrey")) 

eq$arm[eq$arm == 1] <- "Sepsis"
eq$arm[eq$arm == 2] <- "Non-Sepsis"
...
eq$arm <- factor(eq$arm, levels = c("Sepsis", "Non-Sepsis", 3, 4))

  ggplot(subset(eq, arm != 3 & arm != 4 & !is.na(visit_type) ), 
         aes(as.factor(visit_type), vas_score, fill = arm)) +
geom_boxplot(outlier.size = 0.5, alpha = 0.3, width = 0.5) +
theme_bw() +
geom_hline(linetype = "dashed", yintercept = median(subset(eq, arm == 3)$vas_score )) +
    theme(legend.title = element_blank()) +
    labs(y = "VAS score", x= "Week post enrollment") -> p2
