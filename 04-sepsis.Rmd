...
In this chapter, I present a detailed description of the presentation, aetiology, and outcome of sepsis in Blantyre, and an analysis of the determinents of mortality.
...
p$Variable[p$Variable == "fluid_6hr"] <- "IV fluid (ml)"
...
```{r mort-strat-tb-and-tb-rx, echo =F, warning=F, message=F,fig.scap="28-day mortality stratified by TB status and reciept of TB therapy.", out.extra='', fig.cap="Crude 28 day mortality stratified by TB status and reciept of antitubercular therapy: association of TB therapy with survival is not restricted to participants in whom a diagnosis of TB was made.", fig.width=5 , fig.height =3 , out.width ='50%' , fig.align = 'center'}
...
ggplot(t, aes(as.factor(any_tb_rx), prop, ymin = lci, ymax = uci, color = as.factor(tb))) + geom_point(position = position_dodge(width = 0.2)) + geom_errorbar(width = 0, position = position_dodge(width = 0.2)) + coord_flip() + theme_bw() + ylab("Pr(Death by 28 days)") + xlab(element_blank()) + theme(legend.title = element_blank())
