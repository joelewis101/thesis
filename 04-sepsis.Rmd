...
kable(ab.mod.tbl, "latex", booktabs =T,  
      row.names = F, longtable = F, ,escape = F,
      caption = "Coefficient values (expressed as odds ratios) for models predicting death by 28 days that are linear and polynomial in time to antibacterials", col.names = c("Coefficient", "Linear", "Polynomial") ) %>%
...
ggplot(me.all, aes(time_to_abx, estimate__, ymin = lower__, ymax = upper__, color = imputed, fill = imputed )) +
         geom_line() + geom_ribbon(alpha = 0.3, color = NA) + facet_wrap(~ regression) + theme_bw() + xlab("Time to antibacterials (hrs)") + ylab("Pr(Death by 28 days)") + theme(legend.title = element_blank())
