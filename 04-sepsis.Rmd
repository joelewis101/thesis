 ...
 kable_styling("striped", full_width = F, latex_options = c("repeat_header")) %>%
  ...
  pack_rows("Host Variables", 1,6, bold = F) %>% 
   pack_rows("Severity Variables", 7, 21, bold = F)  %>%
    pack_rows("Diagnosis", 25,26, bold = F) %>% 
...
FAMD(d.complete[c( "d28_death", "tb","bc", "csf", "malaria", "screentemp", "t0map", "t0hr", "t0rr", "t0spo2", "gcs","lactate_value",
    "WCC" , "Platelets", "Urea",  "Creatinine", "CO2", "ustand")], sup.var = c(1:5), graph = FALSE) -> sev.famd

FAMD(d.complete[c( "d28_death", "tb","bc", "csf", "malaria", "hivstatus", "CD4.dic", "calc_age", "ptsex","Haemoglobin")], sup.var = c(1:5), graph = FALSE) -> host.famd

fviz_famd_ind(host.famd, habillage = c("d28_death","tb", "malaria", "csf", "bc"), axes = c(1,2), geom = "point",alpha = 0.7)

fviz_famd_var(sev.famd, axes = c(1,3), shape.var = "circle", repel = TRUE) + theme_bw()

as.data.frame(host.famd$ind$coord) -> hostcoords
names(hostcoords) <- paste0("Host.", names(hostcoords))
d.complete <- cbind(d.complete, hostcoords)

as.data.frame(sev.famd$ind$coord) -> sevcoords
names(sevcoords) <- paste0("Sev.", names(sevcoords))
d.complete <- cbind(d.complete, sevcoords)

glm(as.numeric(d28_death) ~ Host.Dim.1+ Sev.Dim.1+ Sev.Dim.2+ bc+ tb+ malaria + 
      any_tb_rx + any_fung_rx + any_mal_rx + fluid_6hr, data = d.complete, family = "binomial") -> m.glm
...
brms::brm(as.numeric(d28_death) ~ Host.Dim.1 +
           Sev.Dim.1 + Sev.Dim.2 + tb + 
           malaria + bc + csf + any_tb_rx + 
          any_fung_rx + 
           any_mal_rx + fluid_6hr, data = d.complete, , family = "bernoulli", prior = m1priors) -> m.b
```
