...
In this chapter, I develop time-inhomogeneous Markov models to model ESBL-E carriage and fit them using the Bayesian probabalistic programming language Stan. I demostrate that fitting these models is feasible with modest computational requirements, and that they are very flexible. I find that hospitalisation acts to increase both rate of ESBL-E acquistion and loss, with a net effect of increasing ESBL-E carriage prevalence over time and that antibacterial exposure acts to prolong ESBL-E carriage by reducing the rate of ESBL-E loss. However, it is the synergistic effect of hospitalisation and antibacterial exposure that seems to drive the rapid increase in ESBL-E carriage prevalence observed in antibacterial-exposed inpatients. The data support a post-exposure effect of antibacterials, such that they continue to have an effect long after they would be expected to be excreted from the body. I present hypotheses about the mechansim of such an effect along with the implications of my findings for antimicrobial stewardship interventions, and planned further work.

## Introduction and chapter aims

In Chapter 5, I presented the longitudinal ESBL-E carriage data for the three arms of the clinical study that underpin this thesis. Antibacterial-exposed, hospitalised participants (arm 1) showed a rapid increase in ESBL-E carriage prevalence, whilst antibacterial-unexposed hospitalised participants (arm 2) showed a much more modest increase. This suggests that antimicrobial exposure is the most significant determinent of acquisition of carriage; however, this unadjusted analysis is open to confounding. The participants recruited to the two arms of the study differ in important characteristics: antimicrobial unexposed participants are younger, less likely to be HIV-infected, with less cotrimoxazole preventative therapy (CPT) exposure, and crucially, a shorter length of hospital stay. An attempt to adjust for potential confounders using simple logistic regression models failed; in this chapter I develop longituinal models to quantify the relative roles of antibacterial exposure and hospitalisation in driving ESBL-E carriage.

There have been few previous attempts to model longitudinal ESBL-E carriage, all from the netherlands: by fitting a Weibull distribution to community sample data[@Teunis2018], by fitting a beta-distribution of admission and discharge carrage probability to data from trials of contact precautions in Dutch hospitals[@Kluytmans-vandenBergh2018], and by modelling household ESBL-E acqusition as a Markov process[@Haverkate2017]. None of these studies included the effect of antibacterial exposure. The Markov model approach is an attractive method to model multistate interval censored data[@Hout2016], and is implemented in the _msm_ package in R[@Jackson2011a] where a maximum likelihood method is used to fit the models. However, _msm_ assumes stepwise-constant covariate effects, for reasons of computational efficiency; there is no reason to assume that the effect of, say, antibacterial exposure will act in this fashion. The aims of this chapter, therefore, are :

* To generalise _msm_-type models to allow true time-varying covariates.
* To demostrate the feasibility of fitting such models.
* To used the fitted models to infer an unbiased estimate of the relative roles of antibacterial exposure and hospitalisation in driving ESBL-E carriage by both fitted parameter estimates and simulating different levels of exposure.  
* To combine the models with ESBL-E species data and with the WGS isolate typing presented in Chapter 7 to explore carriage at the level of species, _E. coli_ clone and ESBL-containing mobile genetic element.

## Methods

### Developing the models use din this chapter
...
The parameter estimates for the models are shown in Figure \@ref(fig:ESBL-mod-param-est). There were significant correlations between some posterior parameters (see chapter Appendix for pairwise plots for model 2 as an example): particularly $\lambda$ and $\mu$, and the $\alpha$ and $\beta$ parameters. This is not necessarily problematic in that it is not necessarily a source of bias, but can make it difficult for some MCMC alogrithms (e.g. Metropololis-Hastings) to adequately sample from the posterior. Nevertheless, the diagnostics suggest that the Stan NUTS sampler had no problems.
