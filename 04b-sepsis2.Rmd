...
Finally, to attempt to correct for confounding using a different method, a propensity-score matching approach was used to produce an unbiased estimate of the effect of receipt of TB therapy on 28-day mortality. Variables that had been identfied as being associated with mortality from the models described above, along with variables that were associated with receipt of TB therapy apparent on bivariable analysis were included in a logistic regression model to generate a propensity score. Because HIV-uninfected participants did not have a CD4 count measured, a new dichotomous variable was used which was coded as 1 for HIV-infected participants with a CD4 count below 100 cells $\mu L^{-1}$ and 0 for everyone else. Participants were then matched 1:1 on this propensity score using the _MatchIt_ package in R and the distribution of covariates in this new cohort examined using kernal denisty plots and histograms. Effect of TB therapy on mortality was then expressed as risk ratios, and subgroup analysis carried out to explore whether there was any effect modification of the apparent effect of TB therapy in advanced immunosupression (defined as CD4 cell count below $\mu L^{-1}$), anaemia (defined as haemoglobin below 8g dL$^{-1}$) or confirmed TB.


## Results

Bayesian logistic regression with weakly informative priors succeding in fitting the models from Chapter 3; the inferences - particularly concerning the apaprent association between TB therapy and survival - were largely unchanged, including after muliple imputation of missing data (Table \@ref(tab:bayes-lr-imp-tab)). Because of concerns about overfitting, dimensionality reduction with FA<D was carried out; the first 3 FAMD dimensions explained 34% of the variance in the dataset, a not inconsiderable amount for 18 predictor variables. The composition of FAMD dimensions one,two and three are shown in Figure \@ref(fig:pca-1)A-B expressed as a plot of the squared correlation ratio (for categoriacal variables) and the squared correlation coefficient (for continuous variables) for each of the variables included in the analysis. Graphically, FAMD dimension one appeared to show an assocation with mortality (Figure \@ref(fig:pca-1)C-D.
...
kable(select(lr_mod,term,str_cc, str_imp), "latex", booktabs =T, 
 ...
 kable_styling("striped", full_width = F, latex_options = c("repeat_header")) %>%
  ...
  footnote(general = "BP = Blood pressure, GCS = Glasgow coma scale, BSI = Bloodstream infection, TB = tuberculosis.  All odds ratios are for as increase in the variables shown." , threeparttable = T, fixed_small_size = T) 
...
ggplot(d.complete2, aes(Dim.1, Dim.3, color = d28_death)) + geom_point(alpha = 0.7) + theme_bw() + geom_hline(yintercept =0,linetype = "dashed", alpha = 0.7) + geom_vline(xintercept = 0, linetype = "dashed", alpha = 0.7) + scale_color_manual(labels = c("Survived", "Died"), values = c("grey", "red")) + theme(legend.title = element_blank()) -> p.6
...
Below I show bivariable associations of reciept of TB therapy and variable distributions of the propensity-score matched cohort. 
...
```{r var-match-plots, echo = F, warning=F, message=F,fig.scap= "Variable distributins following propensity score matching. ", out.extra='', fig.cap="Variable distributins following propensity score matching. A: original cohort. B: Propesnity score matched cohort. BSI = bloodstream infection, RR = respiratory rate, Sp02 = Capilliary oxygen saturation, TB = tuberculosis. Categrorical variables (Malaria rx, BSI, CD4 < 100, Meningitis, HIV, Malaria, TB, Can't stand) are coded as 1 for present and 0 for absent. ", fig.width=8 , fig.height =10  , fig.align = 'center' }
