        ...
        // print(cov_mat)
         ...
         for (n in 1:n_covs) {
             ...
             if (cov_mat[f] != 999) {   //if there is exposure of this covariate in this block
               ...
               out_vars[n] = exp(-(t-cov_mat[f])*gamma);
               ...
               out_vars[n] = exp(-(t-cov_mat[p])*gamma);
               } else {
                     //  otherwise set to 0
                    out_vars[n] = 0; 
               }
   ...
   //      real
    //  ) 
      ...
      //   print("coefs")
       ...
       coefs = return_time_varying_coefs_exp_flat(x_r, t, x_i[1], gamma);
      //   print(coefs);
       // coefs = return_time_varying_coefs_exp()
         
         lambda = lambda0*exp(dot_product(coefs, betas));
         mu = mu0*exp(dot_product(coefs, alphas));
...
real[] test_print_array_with_indices(real[,,] array) {
   int index1;
   int index2;
   int index3;
   int dim1;
...
int r, int c, int s, int n) {
   ...
   print(array[r,c,s])
   print(array[n,,]);
   mat = to_matrix(array[n,,]);
   print("matrix");
   print(mat);
   flat = to_array_1d(mat);
   print(flat);
   arrayagain[] = to_array_2d(to_matrix(flat,3,3,1));
   print(arrayagain);
   return flat;
}

real[] test_flatten_matrix(matrix[,,] array,
int r, int c, int s) {
  // real flat[9];
   //real arrayagain[3,3];
   real whatevz[1];
...
//   print(arrayagain);
 ...
 model {
     ...
     temp = integrate_ode_bdf(twostateODE2_flat, start_state[n], 0, t[n:n], theta, cov_mat[n], temp_ncovs[], 1e-7,1e-5,1e6);
      if (end_state[n] == 1) {
      target += log(temp[1,2]);
      } else {
      target += log(temp[1,1]);
