...
ggsave(paste0(wd,"aminoglycoside_plot.pdf"), plot = parrange, width = 20, height = 10, units = "cm")
...
```
#### Chloramphenicol, co-trimoxazole and tetracycline resistance
...
names(cl.sens)[names(cl.sens) == "Chloramphenicol"] <- "Chloramphenicol resistance"

cl.sens <- cl.sens[order(cl.sens$Chloramphenicol, cl.sens$CatB4, cl.sens$CatA1, cl.sens$CmlA1, cl.sens$CatB3),]
...
ggsave(paste0(wd,"chor_cotrim_plot.pdf"), plot = parrange, width = 20, height = 10, units = "cm")
```

```{r wgs-amr-chlor-cotrim, echo = F, warning = F, message = F, fig.scap="Chloramphenicol and cotrimoxazole resistance mutations and phenotypic resistance", out.extra='', fig.cap="Heatmap showning phenotypic chlramphenicol (A) and cotrimoxazole (B) resistance and identified resistance genes that could be expected to confer resistance to these agents. Each row is one sample, red = presence, blue = absence"}

knitr::include_graphics(paste0(wd,"chor_cotrim_plot.pdf"))

```

### Lineage association and clustering of AMR determinents

Next, I explored associations of AMR determinents, both with bacterial lineages, and with other AMR determinents. First, I mapped the identified AMR determinents back to the maximum likelihood tree (Figure \@ref(fig:wgs-amr-maptotree)), demonstrating a clear lineage association, in that certain AMR genes tend to cluster within lineages. To explore this further, the matrix of pairwise jaccard indices was constructed 

```{r wgs-amr-maptotree, echo = F, warning = F, message = F, fig.scap="AMR determinents mapped back to tree", out.extra='', fig.cap="AMR determinents mapped back to tree, showing lineage associations of AMR genes and plasmid replicons. Presence of QRDR mutations is indicated by blue (any nonsynonymous mutation in any QRDR shown), plasmid replicons purple and other AMR genes red.", fig.width = 12, fig.height = 8}
...
color = "lightgrey", width = 3, offset = 0.02)  + scale_fill_manual(values = c("white", "red", "purple", "blue")) +
...
```{r wgs-amr-jaccmat, echo = F, warning = F, message = F, fig.scap="Co-occurance of AMR genes expressed as Jaccard index", out.extra='', fig.cap="Heatmap of row and column clustered matrix of pairwise Jaccard indices to compare co-occurance of AMR genes", fig.width = 5, fig.height = 5}
...
pheatmap(jcoefs, fontsize = 5, color = viridis(n = 100, option = "inferno"), border_color = NA)


```

```{r wgs-amr-cooccurnet, echo = F, warning = F, message = F, fig.scap="Co-occurance of AMR genes expressed as Jaccard index", out.extra='', fig.cap="Heatmap of row and column clustered matrix of pairwise Jaccard indices to compare co-occurance of AMR genes", fig.width = 5, fig.height = 5}
...
### Testing metadata associations: Sequence clusters and ESBL-clusters
