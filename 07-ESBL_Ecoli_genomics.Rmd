...
```{r wgs-contig-ggenes-plot1, echo = F, warning = F, message = F, fig.scap="Structure of ESBL-clusters, part 1", out.extra='', fig.cap="AMR genes, insertion sequences (IS) and plasmid replicons identified in the representative contig of each contig cluster, stratified by by ESBL gene and ordered by size of cluster. IS26 is very frequently associated with a 108bp fragment of \\textit{catB4} chloramphenicol resistance gene, shown as a red fragnemt within the green IS26 element. A: \\textit{blaCTXM15}, B: \\textit{blaCTXM27} , C: \\textit{blaSHV12}. Plots show furthest IS/AMR gene or plasmid replicon up to +/- 10,000bp from the ESBL gene of interest.", fig.width = 13, fig.height = 15}
...
ggtree(tree) %>% gheatmap(df.onehot[-1],  font.size = 2,
                          colnames_position = "top", colnames_angle = 90, colnames_offset_y = 20,
                          width = 3, color = NA) + scale_fill_manual(values = c("lightgrey", "black", "white"))
...
Having clustered bacteria and MGE using _hierBAPS_ and _cd-hit_ respectively, I then used the _hierBAPS_ SCs to assess for healthcare associated lineages.
