...
ggplot(dftemp, aes(name, value, fill = variable)) + geom_col() + coord_flip() + scale_fill_brewer(palette = "Paired") + theme(axis.text.y = element_text(size = 2), legend.title = element_blank(), legend.text = element_text(size = 8)) + labs(x= "Sample ID", y = "% reads" ) 
...
```{r wgs-mlst, echo = F, warning = F, message = F, fig.cap="\\textit{E. coli} sequence type distribution", fig.height = 4, fig.width=5}

ggplot(mlst, aes(fct_infreq(ST))) + geom_bar() + coord_flip()  +
  xlab("E. coli sequence type") + ylab("Count") + theme(axis.text=element_text(size=5))
...
```{r wgs-amr-sum, echo = F, warning = F, message = F, fig.scap="Frequency distribution of AMR genes", out.extra='', fig.cap="Frequency distribution of AMR genes", fig.height = 7, fig.width=4}
...
ggplot(amr.class, aes(class, fill = as.character(class))) + geom_bar() +theme_bw() + theme(axis.text.x = element_text(angle = 90), legend.position = "none", axis.title = element_blank()) -> p2

ggarrange(p1, ggarrange(p2, NULL, ncol = 1, nrow = 2, labels =c("B","C")), ncol = 2, nrow = 1, widths = c(1.5,1), labels = c("A", NA))
