...
```{r wgs-amr-sum, echo = F, warning = F, message = F, fig.scap="Frequency distribution of AMR genes", out.extra='', fig.cap="Frequency distribution of AMR genes", fig.height = 7, fig.width=5}
...
amr.melt$variable<- factor(amr.melt$variable, levels = classlevels$variable[order(classlevels$class, classlevels$n, decreasing = TRUE)])

ggplot(amr.melt, aes(variable, fill = class)) + geom_bar() + coord_flip() + theme(axis.text.y = element_text(size = 5))
