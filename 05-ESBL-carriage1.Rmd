...
kable(select(t1,value, str1, str2, str3, p,Total), "latex", booktabs =T, 
      ...
      caption = "Participant characteristics" ) %>%  kable_styling(font_size = 9, full_width = F) %>%
...
lims_sens %>% 
  ...
  mutate(name = recode(name, Meropenam = "Meropenem")) %>%
  ...
  theme_bw() + 
  theme(legend.title = element_blank(),legend.position = "bottom", axis.text = element_text(size = 8),strip.text = element_text(face = "italic")) + 
  xlab(element_blank()) +
  ylab("Proportion") -> psens

ggarrange(p.org, psens, ncol = 1, nrow = 2, heights = c(1,2), labels = c("A", "B"))
```
