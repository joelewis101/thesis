...
```{r ESBL-spec-sens, echo = F, warning = F, message = F,fig.scap="Species and antimicrobial sensitivities of cultured ESBL-E", out.extra='', fig.cap = "Species (A) and antimicrobial sensitivities (B) of cultured ESBL-E",  fig.height = 4, fig.width=6, fig.align = 'center'}
...
lims_orgs %>% 
  ...
  xlab(element_blank()) + ylab("n") + theme_bw() -> p.org
...
bl.esbl %>% select(d2arm, calc_age, ptsex, hivstatus, hivonart, hivcpt, 
                   recieved_prehosp_ab,pmhxrechospital,
...
glm(ESBL ~ ., data = bl.esbl, family = 'binomial') -> mv.mod
  ...
  dfout$str <- dfout %>% glue_data('{specify_decimal(estimate, 2)} ({specify_decimal(conf.low, 2)}-{specify_decimal(conf.high, 2)})')
    ...
    boldrows <- bl.esbl.tab$mv_p.value < 0.05
