// Stan model for msm style interval censored model, on real data, exponential function
   ...
   //      real
    //  ) 
      ...
      //   print("coefs")
       ...
       coefs = return_time_varying_coefs_exp_flat(x_i[2:1+3*x_i[1]], t, x_i[1], gamma);
      //   print(coefs);
  ...
  data {
      ...
      real t[N]; // end time
      int cov_mat[N,1+3*n_covs]; // array of covariates
 ...
 model {
     ...
     temp = integrate_ode_rk45(twostateODE2_flat, start_state[n,], 0, t[n:n], theta, x_r[] , cov_mat[n,], 1e-7,1e-5,1e6);
      if (end_state[n] == 1) {
      target += log(temp[1,2]);
      } else {
      target += log(temp[1,1]);
