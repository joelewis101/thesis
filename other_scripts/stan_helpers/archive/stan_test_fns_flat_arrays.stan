    ...
    //  ) 
      ...
      //   print(coefs);
       // coefs = return_time_varying_coefs_exp()
         ...
         lambda = lambda0*exp(dot_product(coefs, alphas));
         mu = mu0*exp(dot_product(coefs, betas));
...
//   print(arrayagain);
 ...
 model {
     ...
     temp = integrate_ode_bdf(twostateODE2_flat, start_state[n], 0, t[n:n], theta, cov_mat[n], temp_ncovs[]);
      if (end_state[n] == 1) {
      target += log(temp[1,2]);
      } else {
      target += log(temp[1,1]);
