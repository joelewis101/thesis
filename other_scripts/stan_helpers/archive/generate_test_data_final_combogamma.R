...
as.matrix(cbind(m1,m2)) -> covs_array
...
odeR <- function(t, state, parameters) {
  coefs <- return_time_varying_coefs_exp_flat (parameters, t, c(3,2), gamma)
  # print(coefs)
...
for (i in 1:n_pat) {
   finalstate.prob [i] <- ode(c(p0[i], p1[i]), c(0,t[i]), odeR, covs_array[i,])[[2,2]]
}

lapply(finalstate.prob, function(x) rbinom(1,1,x)) -> finalstate
finalstate <- unlist(finalstate)

stan_test_data <- list(N = n_pat, n_covs = c(0,1,1), covs_type = c(3,2) ,t = t, cov_mat = covs_array, start_state = initstates, end_state = finalstate)

stanmod<- "other_scripts/stan_helpers/stan_final_rk45_combovarygammas.stan"
saveRDS(stan_test_data, "chapter_9/test_data/test_data_4.rda")

test_fit <- stan(stanmod, data = stan_test_data, warmup = 1, iter = 1, chains = 1, cores = 1, thin = 1)
